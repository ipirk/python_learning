...
def add_to_index(index,keyword,url):
    for sublist in index:
        if sublist[0] == keyword:
            for urls in sublist[1]:
                if urls[0]==url:
                    return
            sublist[1].append([url,0])
            return   
...
def split_string2(string, splitlist):
    size=len(string)
    i=0
    words=[]
    word=""
...
add_to_index(index,"test2","url1")
add_to_index(index,"test3","url2")
add_to_index(index,"test3","url1")
...
#print get_page1("http://xkcd.com/353")
